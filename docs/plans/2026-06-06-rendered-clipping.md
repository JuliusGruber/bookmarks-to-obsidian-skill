...
- **Post-ship fix (2026-06-07) — the double-parse was abandoned for the rendered path.** Deviation #1 kept a node re-parse (`extractFromHtml(r.content)`) of the in-page-cleaned HTML. A render-vs-fetch A/B surfaced that this re-parse silently **drops div-wrapped inline images** (x.com article: 12 → 1) — node Defuddle, re-extracting a context-free fragment without an `<article>` root, scores the image blocks out (and can even return `skipped-thin`). The equivalence test never caught it because it only checks *text* on an `<article>` fixture. Fix (exactly as deviation #1 prescribed): the rendered path now converts to markdown **in-page** via `parse({markdown:true})` in `render.mjs` (which preserves all images), and `import.mjs` uses `r.content` + `r.wordCount` directly — no node re-parse. The fetch path still uses `extractFromHtml`. Guarded by a new `RENDER_SMOKE` test (`returns markdown preserving every div-wrapped inline image`). Verified end-to-end: the x.com note went 1 → 12 local image embeds.
```
